...
- IRAS/REC/version number/title for all ethics pack documents autogenerated by
  CRATE. Remove ``PDF_LETTER_HEADER_HTML`` and ``PDF_LETTER_FOOTER_HTML`` from
...
- Fix the installer so that sensible Django Database settings are written for
  SQL Server databases accessed via DSN in the ODBC configuration.
  https://github.com/ucam-department-of-psychiatry/crate/issues/159

- Fix access to /crate/files from the Flower container.
  https://github.com/ucam-department-of-psychiatry/crate/issues/162

- URL-encode passwords that appear in SQL Alchemy URLs generated by the
  installer. Before this change passwords with special characters were resulting
