...
def lookup_cpft_crs(
    ...
    if not row:
        ...
        lookup.set_gp_name_components(
            row["gpname"] or "", decisions, secret_decisions
        )
        ...
        lookup.gp_address_5 = ", ".join(
            [row["address4"] or "", row["address5"] or ""]
        )
    ...
    if not row:
        ...
        careco_email = row[
            "carecoordinatoremailaddress"
        ] or make_cpft_email_address(
            row["carecoordinatorfirstname"], row["carecoordinatorlastname"]
        )
