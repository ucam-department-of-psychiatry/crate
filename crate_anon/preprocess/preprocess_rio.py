...
.. todo:: preprocess_rio:
    specific supposed PK failing (non-unique) on incremental
...
.. todo:: preprocess_rio:
    Similarly, all cross-checks to RCEP output (currently
...
def table_is_rio_type(
    tablename: str, configoptions: RioViewConfigOptions
) -> bool:
    """
...
def get_rio_patient_id_col(table: Table) -> str:
    ...
    """
    patient_id_col = RIO_6_2_ATYPICAL_PATIENT_ID_COLS.get(
        table.name, RIO_COL_PATIENT_ID
    )
...
def process_patient_table(
    table: Table, engine: Engine, configoptions: RioViewConfigOptions
) -> None:
    """
    ...
    cast_id_to_int = sql_fragment_cast_to_int(
        string_pt_id, dialect=engine.dialect
    )
...
def process_master_patient_table(
    table: Table, engine: Engine, configoptions: RioViewConfigOptions
) -> None:
    """
    ...
    Args:
        ...
        configoptions: instance of :class:`RioViewConfigOptions`
    """
    crate_col_nhs_number = Column(
        CRATE_COL_NHS_NUMBER, BigInteger, nullable=True
    )
...
def drop_for_progress_notes(table: Table, engine: Engine) -> None:
    ...
    drop_indexes(
        engine,
        table,
        [CRATE_IDX_RIONUM_NOTENUM, CRATE_IDX_MAX_SUBNUM, CRATE_IDX_LAST_NOTE],
    )
    drop_columns(engine, table, [CRATE_COL_MAX_SUBNUM, CRATE_COL_LAST_NOTE])


def process_clindocs_table(
    table: Table, engine: Engine, configoptions: RioViewConfigOptions
) -> None:
    """
...
def drop_for_clindocs_table(table: Table, engine: Engine) -> None:
    ...
    drop_indexes(
        engine,
        table,
        [CRATE_IDX_RIONUM_SERIALNUM, CRATE_IDX_MAX_DOCVER, CRATE_IDX_LAST_DOC],
    )
    drop_columns(engine, table, [CRATE_COL_MAX_DOCVER, CRATE_COL_LAST_DOC])
...
def get_rio_views(
    ...
    for viewname, viewdetails in RIO_VIEWS.items():
        ...
        if basetable.lower() not in all_selectables_lower:
            log.warning(
                f"Skipping view {viewname} as base table/view "
                f"{basetable} not present"
            )
            continue
        suppress_basetable = viewdetails.get(
            "suppress_basetable", suppress_basetables
        )
...
def add_postcode_geography_view(
    engine: Engine, configoptions: RioViewConfigOptions, ddhint: DDHint
) -> None:
    """
    ...
    orig_column_names = get_column_names(
        engine, tablename=addresstable, sort=True
    )
    ...
    orig_column_specs = [f"{addresstable}.{col}" for col in orig_column_names]
    geog_col_specs = [
    ...
    ensure_columns_present(
        engine, tablename=addresstable, column_names=[rio_postcodecol]
    )
    colsep = ",\n            "
    select_sql = """
        SELECT {origcols},
    ...
    assert_view_has_same_num_rows(
        engine, addresstable, VIEW_ADDRESS_WITH_GEOGRAPHY
    )
...
def process_table(
    ...
    is_patient_table = (
        patient_table_indicator_column in column_names
        or tablename == configoptions.full_prognotes_table
    )
...
def process_all_tables(
    engine: Engine, metadata: MetaData, configoptions: RioViewConfigOptions
) -> None:
    """
...
*   By default, this treats the source database as being a copy of a RiO
    ...
    log.info(
        "CRATE in-place preprocessor for RiO or RiO CRIS Extract Program "
        "(RCEP) databases"
    )
