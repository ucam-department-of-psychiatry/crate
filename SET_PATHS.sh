...
export PIP=$CRATE_VIRTUALENV/bin/pip
export PYTHON=$CRATE_VIRTUALENV/bin/python
export CRATE_BASE=$THIS_SCRIPT_DIR
export CRATE_DJANGO_ROOT=$CRATE_BASE/crateweb
export CRATE_MANAGE="$PYTHON $CRATE_DJANGO_ROOT/manage.py"

add_pythonpath $THIS_SCRIPT_DIR
# add_pythonpath $THIS_SCRIPT_DIR/crateweb
add_pythonpath $THIS_SCRIPT_DIR/secret_local_settings
PYTHONBASE=`find "$CRATE_VIRTUALENV/lib" -name "python*" | head -1`
# ... will include virtualenv path
cat << ENDMESSAGE
PYTHONPATH is now: $PYTHONPATH

To use Apache with mod_wsgi, use a section like this in the Apache config
file (example with 5 processes):

    WSGIDaemonProcess SOME_NAME processes=5 threads=1 python-path=$PYTHONBASE/site-packages
    WSGIScriptAlias /MY/MOUNT/URL $THIS_SCRIPT_DIR/crateweb/config/wsgi.py process-group=SOME_NAME application-group=%{GLOBAL}

Additionally, install mod_xsendfile, e.g. (on Ubuntu):
    $ sudo apt-get install libapache2-mod-xsendfile
... which will implicitly run "a2enmod xsendfile" to enable it. Then add to the
Apache config file in an appropriate place:
...
Use \$CRATE_MANAGE as the Django management tool, e.g. for the demo server:
