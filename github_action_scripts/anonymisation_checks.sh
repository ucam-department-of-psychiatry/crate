...
anonymised=$(docker compose exec -T research_db mysql -Ns -u ${CRATE_DOCKER_RESEARCH_DATABASE_USER_NAME} -p${CRATE_DOCKER_RESEARCH_DATABASE_USER_PASSWORD} ${CRATE_DOCKER_RESEARCH_DATABASE_NAME} -e "SELECT SUBSTR(note, 1, 39) FROM note LIMIT 1")
